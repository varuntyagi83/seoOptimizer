...
export function PageList({ pages, onSelect, selectedUrl }: PageListProps) {
  ...
  function SortHeader({ label, col }: { label: string; col: SortKey }) {
    ...
    return (
      ...
      >
        {label}
        <span className="text-slate-600">{active ? (dir === 'asc' ? '↑' : '↓') : '↕'}</span>
      </button>
    )
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-800">
        <table className="w-full text-sm">
          <thead className="border-b border-slate-800 bg-slate-900/60">
            <tr>
              ...
              <th className="text-right px-3 py-2.5 text-xs font-semibold uppercase tracking-wider text-slate-500">Issues</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((page, i) => {
                ...
                <tr
                  ...
                  className={`border-b border-slate-800/50 cursor-pointer transition-colors ${
                    isSelected
                      ...
                      : i % 2 === 0 ? 'bg-transparent hover:bg-slate-800/30' : 'bg-slate-900/20 hover:bg-slate-800/30'
                  }`}
                >
                  <td className="px-4 py-2.5 font-mono text-xs text-slate-300 max-w-xs truncate">
                    {page.url.replace(/^https?:\/\//, '')}
                    {page.depth === 0 && (
                      <Badge variant="outline" className="ml-2 text-[10px] border-cyan-800 text-cyan-600 py-0">home</Badge>
                    )}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right">
                    {issues > 0 ? (
                      <span className="text-xs text-red-400 font-semibold">{issues}</span>
                    ) : (
        ...
        {sorted.length === 0 && (
          <div className="py-8 text-center text-slate-600 text-sm">No pages match the filter</div>
        )}
      </div>
    </div>
  )
