...
const SEVERITY_STYLES = {
  ...
  warning:  'text-yellow-400 bg-yellow-950/40 border-yellow-900/50',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-1">
        <p className="text-slate-500 text-sm font-mono">{domain}</p>
        <p className="text-slate-400 text-xs">{stats.totalPages} pages crawled</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <div key={label} className="bg-slate-900/60 border border-slate-800 rounded-lg p-3 space-y-1">
            <p className="text-xs text-slate-500">{label}</p>
            <p className={`text-lg font-semibold tabular-nums ${warn ? 'text-yellow-400' : 'text-slate-200'}`}>
              {value}
            </p>
          </div>
        ))}
      ...
      {allIssues.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Site-Wide Issues</h3>
          <div className="space-y-2">
            {allIssues.map((issue) => (
              <div
                key={issue.type}
